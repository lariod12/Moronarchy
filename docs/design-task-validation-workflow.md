...
| `Reset Room` / Reload | Round, players, phase, dice face, initial ownership, selected scenario, debug setup, and in-flight autoplay work reset. | Both tabs immediately show a clean 2-player board with no owned land, all players on Start, `100` coin per player, no selected scenario, and paused autoplay until `Play`. |
